...
export default function RegisterPage() {
  ...
  return (
    <div className="w-full aurora-bg relative z-10 overflow-hidden py-12" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}>
      <SharedContainer className="flex flex-col items-center justify-center w-full">
        ...
        <motion.div className="w-full max-w-[520px] flex flex-col items-center" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <Spacer h={4} />
            <p className="text-sm" style={{ color: 'rgba(248,250,252,0.5)' }}>Create your account</p>
          </div>
          ...
          <div className="glass-card w-full p-[48px] flex flex-col">
            <form onSubmit={handleSubmit} className="w-full flex flex-col gap-[24px]">
              ...
              <div className="w-full flex flex-col gap-[8px]">
                <label className="text-xs font-medium" style={{ color: 'rgba(248,250,252,0.6)' }}>Full Name</label>
                <div className="relative w-full">
                  <User size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: 'rgba(0,212,255,0.5)' }} />
                  <input type="text" className="w-full pl-[36px] pr-3 py-[12px] text-sm rounded-lg outline-none" placeholder="Your name"
              ...
              <div className="w-full flex flex-col gap-[8px]">
                <label className="text-xs font-medium" style={{ color: 'rgba(248,250,252,0.6)' }}>Role</label>
                <div className="grid grid-cols-3 gap-[8px]">
                  {ROLES.map(r => (
                    <button key={r} type="button" onClick={() => setRole(r)}
                      className="py-[10px] rounded-lg text-xs font-medium capitalize transition-all outline-none"
                      style={{
                        background: role === r ? 'rgba(0,212,255,0.2)' : 'rgba(9,28,47,0.8)',
                        border: `1px solid ${role === r ? 'rgba(0,212,255,0.5)' : 'rgba(0,212,255,0.15)'}`,
                        color: role === r ? '#00D4FF' : 'rgba(248,250,252,0.5)'
                      }}>{r}</button>
                  ))}
                </div>
              </div>
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <div className="loading-dots"><span /><span /><span /></div> : <><Zap size={16} /> Create Account</>}
              </button>
            </form>
            ...
            <div className="w-full flex flex-col items-center gap-[12px]">
              <p className="text-sm" style={{ color: 'rgba(248,250,252,0.4)' }}>
                ...
                <Link to="/login" style={{ color: '#00D4FF' }} className="font-semibold hover:underline">Sign In</Link>
              </p>
              <p className="text-sm" style={{ color: 'rgba(248,250,252,0.4)' }}>
